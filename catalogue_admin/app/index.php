<!DOCTYPE html>
<html lang="en">

<?php 
session_start();
if(!$_SESSION['nama']){
  header('Location: ../index.php?session=expired');
}
include('header.php'); ?>
<?php include('../conf/config.php'); ?>

<body class="hold-transition sidebar-mini layout-fixed">
<div class="wrapper">

  <!-- Preloader -->
  <?php include('preloader.php'); ?>

  <!-- Navbar -->
  <?php include('navbar.php'); ?>
  <!-- /.navbar -->

  <!-- Main Sidebar Container -->
  <aside class="main-sidebar sidebar-dark-primary elevation-4">
    <!-- Brand Logo -->
    <?php include('logo.php'); ?>

    <!-- Sidebar -->
    <?php include('sidebar.php') ?>
    <!-- /.sidebar -->
  </aside>

  <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <?php include('content_header.php'); ?>
    <!-- /.content-header -->

    <!-- Main content -->
    <?php 
    if (isset($_GET['page'])){
      if ($_GET['page']=='users'){
        include('data/user.php');
      }
      else if($_GET['page']=='tambah-user'){
        include('tambah/tambah_user.php');
      }
      else if($_GET['page']=='edit-user'){
        include('edit/edit_user.php');
      }
      else if($_GET['page']=='data-seragam'){
        include('data/data_seragam.php');
      }
      else if($_GET['page']=='tambah-data-seragam'){
        include('tambah/tambah_data_seragam.php');
      }
      else if($_GET['page']=='edit-data-seragam'){
        include('edit/edit_data_seragam.php');
      }
      else if($_GET['page']=='data-apd'){
        include('data/data_apd.php');
      }
      else if($_GET['page']=='tambah-data-apd'){
        include('tambah/tambah_data_apd.php');
      }
      else if($_GET['page']=='edit-data-apd'){
        include('edit/edit_data_apd.php');
      }
      else if($_GET['page']=='data-customer'){
        include('data/data_customer.php');
      }
      else if($_GET['page']=='tambah-data-customer'){
        include('tambah/tambah_data_customer.php');
      }
      else if($_GET['page']=='edit-data-customer'){
        include('edit/edit_data_customer.php');
      }
      else if($_GET['page']=='data-job'){
        include('data/data_job.php');
      }
      else if($_GET['page']=='tambah-data-job'){
        include('tambah/tambah_data_job.php');
      }
      else if($_GET['page']=='edit-data-job'){
        include('edit/edit_data_job.php');
      }
      else{
        include('not_found.php');
      }
    }
    else{
      include('data/data_seragam.php');
    }?>
    <!-- /.content -->
  </div>
  <!-- /.content-wrapper -->
  <?php include('footer.php'); ?>

  <!-- Control Sidebar -->
  <aside class="control-sidebar control-sidebar-dark">
    <!-- Control sidebar content goes here -->
  </aside>
  <!-- /.control-sidebar -->
</div>
<!-- ./wrapper -->


</body>
</html>
